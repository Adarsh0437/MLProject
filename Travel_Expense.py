...
with st.form("expense_form"):
    ...
    with col1:
        hotel_price = st.number_input("🏨 Hotel Price ($)", min_value=0,max_value=350,step=10)
        from_location = st.selectbox("📍 From Location", from_locations, index=0)
        days = st.slider("📅 Number of Days", min_value=1, max_value=5, value=1)
        flight_type_input = st.selectbox("✈️ Flight Type", flight_type_options)

    with col2:
        flight_price = st.number_input("💺 Flight Price ($)", min_value=0,max_value=2000,step=50)
        to_location = st.selectbox("📍 To Location", to_locations, index=0)
        distance = st.slider("🛣️ Distance (km)", min_value=100,max_value=1000)
